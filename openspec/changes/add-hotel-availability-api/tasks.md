...
- [ ] 3.1.1 Create `PremierStaysStubData` with static catalog Dictionary<Guid, RoomCatalogEntry> for 5+ rooms across India (Mumbai/INR), UK (London/GBP), US (New York/USD), covering all 3 room types
- [ ] 3.1.2 Create `BudgetNestsStubData` with static catalog Dictionary<Guid, RoomCatalogEntry> for 5+ rooms across same locations, different GUIDs
- [ ] 3.1.3 Ensure catalog entries include Provider, Location, Destination, RoomType, Currency based on location rules

### 3.2 Provider Implementations

- [ ] 3.2.1 Implement `PremierStaysProvider`: Search filters catalog by destination (case-insensitive), dates (exact match), room type; returns Room value objects
- [ ] 3.2.2 Implement `PremierStaysProvider.GetRoomById`: lookup GUID in catalog, return Room or null
- [ ] 3.2.3 Create `PremierStaysMapper` to convert catalog entry → Room value object
- [ ] 3.2.4 Implement `BudgetNestsProvider`: Search filters catalog (same logic as PremierStays)
- [ ] 3.2.5 Implement `BudgetNestsProvider.GetRoomById`: lookup GUID in catalog, return Room or null
- [ ] 3.2.6 Create `BudgetNestsMapper` to convert catalog entry → Room value object

### 3.3 Repository Implementation

- [ ] 3.3.1 Implement `InMemoryReservationRepository` using ConcurrentDictionary<string, Reservation> keyed by reference number
- [ ] 3.3.2 Implement Add method (stores reservation)
- [ ] 3.3.3 Implement GetByReference method (retrieves or returns null)

### 3.4 Dependency Injection

- [ ] 3.4.1 Create `ServiceCollectionExtensions` in Infrastructure/DependencyInjection with AddInfrastructure method
- [ ] 3.4.2 Register IReservationRepository → InMemoryReservationRepository (singleton)
- [ ] 3.4.3 Register IHotelProvider → PremierStaysProvider (singleton)
- [ ] 3.4.4 Register IHotelProvider → BudgetNestsProvider (singleton)
- [ ] 3.4.5 Register IHotelSearchService → HotelSearchService (scoped)
- [ ] 3.4.6 Register IReservationService → ReservationService (scoped)
...
- [ ] 4.1.1 Implement `CorrelationIdMiddleware`: read X-Correlation-Id from request header, generate GUID if missing, add to logger scope, add to response header
- [ ] 4.1.2 Implement `ExceptionHandlingMiddleware`: catch DomainValidationException → 400, DocumentMismatchException → 422, ReservationNotFoundException → 404, return ProblemDetails (RFC 7807)
- [ ] 4.1.3 Create `ApplicationBuilderExtensions` with UseCustomMiddleware method to register both middleware
- [ ] 4.1.4 Register middleware in Program.cs (after routing, before endpoints)

### 4.2 Endpoints

- [ ] 4.2.1 Create `GET /hotels/search` endpoint: bind query params to HotelSearchRequest, call HotelSearchService, return 200 with `{ results: Room[] }` (Room serializes directly)
- [ ] 4.2.2 Add validation to search endpoint: required params (destination, checkIn, checkOut, roomType), valid date format, checkOut > checkIn (return 400 ProblemDetails if invalid)
- [ ] 4.2.3 Create `POST /hotels/reserve` endpoint: bind JSON body to ReservationRequest, call ReservationService.Reserve, return 201 with ReservationResponse
- [ ] 4.2.4 Add validation to reserve endpoint: required fields (roomId, document), document number length 5-20 chars (return 400 ProblemDetails if invalid)
- [ ] 4.2.5 Create `GET /hotels/reservation/{referenceNumber}` endpoint: validate reference format (REF-[a-f0-9]{8}), call ReservationService.GetByReference, return 200 with ReservationResponse
- [ ] 4.2.6 Add case-insensitive reference number handling in lookup endpoint

### 4.3 API Configuration

- [ ] 4.3.1 Update Program.cs: add Infrastructure DI registration, configure JSON serialization (camelCase, ignore nulls), add CORS if needed
- [ ] 4.3.2 Update appsettings.json: configure structured logging with correlation ID placeholder
...
- [ ] 6.1 Create `api-testing-guidelines.md` with overview of all 3 endpoints (search, reserve, lookup)
- [ ] 6.2 Add example curl commands for each endpoint with sample request/response payloads to `api-testing-guidelines.md`
- [ ] 6.3 Document how to test endpoints locally (run API, use Postman/curl, validate responses) in `api-testing-guidelines.md`
- [ ] 6.4 Add troubleshooting section for common errors (invalid roomId, document mismatch, missing params, etc.) to `api-testing-guidelines.md`
- [ ] 6.5 Add quick start section: clone → restore → build → run → test first endpoint in `api-testing-guidelines.md`
